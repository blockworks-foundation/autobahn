...
async fn test_dump_input_data_raydium() -> anyhow::Result<()> {
    ...
    raydium_step_2(&options, !disable_compressed).await?;
...
async fn raydium_step_1(
    ...
    let dex =
        dex_raydium::RaydiumDex::initialize(&mut rpc_client, options.clone(), enable_compression)
            .await?;
